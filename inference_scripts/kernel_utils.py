...
def confident_strategy(pred, t=0.8):
    ...
    if fakes > sz // 2.5 and fakes > len(pred)//2:
...
def predict_on_video_set(face_extractor, file_path, input_size, num_workers, frames_per_video, models,
                         strategy=np.mean,
                         apply_compression=False):

    y_pred = predict_on_video(face_extractor=face_extractor, video_path=file_path,
